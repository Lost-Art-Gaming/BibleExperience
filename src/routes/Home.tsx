...
import { cleanTitle, getLastRead, isDone } from '../lib/storage';
    ...
    <>
      {resume && (
        <button className="resume-bar" onClick={() => navigate(`/episode/${encodeURIComponent(resume.id)}`)}>
          <span className="resume-icon"><Icon name="play" /></span>
          <span><small>Continue reading</small><b>{cleanTitle(resume.title)}</b></span>
          <span className="resume-go"><Icon name="arrow" /></span>
        </button>
      )}
      <Reveal>
      ...
      <Reveal delay={80}>
        <section className="content-section journey-progress">
          <div className="section-heading"><div><span className="eyebrow">YOUR JOURNEY</span><h2>Walk through the Bible</h2></div><strong>{pct}%</strong></div>
          <ProgressBar value={pct} />
          <div className="mini-stats">
            <span><b>{doneCount}</b> completed</span>
            <span><b>{total}</b> experiences</span>
          </div>
        </section>
      </Reveal>

      <Reveal delay={120}><HomeTapestryTeaser /></Reveal>

      <Reveal delay={160}>
        <section className="content-section">
          <div className="section-heading"><div><span className="eyebrow">EXPLORE</span><h2>Go beyond the page</h2></div></div>
          <div className="feature-grid">
            <button className="feature-card map-feature" style={artStyle('.map-feature', 1)} onClick={() => navigate('/explore')}>
              <span className="feature-icon"><Icon name="map" /></span><b>Biblical Geography</b><small>Walk the places of Scripture</small><span className="feature-arrow"><Icon name="arrow" /></span>
            </button>
            <button className="feature-card timeline-feature" style={artStyle('.timeline-feature', 2)} onClick={() => navigate('/timeline')}>
              <span className="feature-icon"><Icon name="timeline" /></span><b>Biblical Timeline</b><small>See Jehovah&rsquo;s purpose unfold</small><span className="feature-arrow"><Icon name="arrow" /></span>
            </button>
            <button className="feature-card journey-feature" style={artStyle('.journey-feature', 3)} onClick={() => navigate('/journey')}>
              <span className="feature-icon"><Icon name="journey" /></span><b>Scripture Journey</b><small>Read, reflect and discover</small><span className="feature-arrow"><Icon name="arrow" /></span>
            </button>
          </div>
        </section>
      </Reveal>
